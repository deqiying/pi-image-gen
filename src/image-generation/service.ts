...
export async function executeImageGeneration(args: {
  ...
  const imageModel = loaded.config.imageModel ?? runtime.model;
  ...
  try {
    ...
    const body = buildImageGenerationRequest({ routingModel: runtime.model, imageModel, params, references });
    const response = await deps.requestImage({ runtime, body, ...(loaded.config.userAgent ? { userAgent: loaded.config.userAgent } : {}), ...(args.signal ? { signal: args.signal } : {}) });
    ...
    const artifactPath = await saveCanonicalImage({ bytes: generated, agentDir: deps.agentDir(), sessionId: getSessionId(args.ctx), imageCallId: response.image.imageCallId || args.toolCallId });
    ...
    const details = {
      ...
      routingModel: `${runtime.provider}/${runtime.model}`,
      imageModel,
      imageCallId: response.image.imageCallId,
      ...(response.image.responseId ? { responseId: response.image.responseId } : {}),
